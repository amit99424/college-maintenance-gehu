  ...
  const validateSignup = () => {
    ...
    if (role === "student") {
      if (!/^[a-zA-Z0-9._%+-]+@gmail\.com$/.test(email)) {
        setError(t("studentEmailValidation"));
        return false;
      }
    }
    if (role === "staff") {
      if (!/^[a-zA-Z0-9._%+-]+@staff\.com$/.test(email)) {
        setError(t("staffEmailValidation"));
        return false;
      }
    }
    if (role === "supervisor") {
      if (!/^[a-zA-Z0-9._%+-]+@sup\.com$/.test(email)) {
        setError(t("supervisorEmailValidation"));
        return false;
      }
    }
    setError("");
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden">
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-4 sm:px-6 lg:px-8 py-4">
        <div className="w-[90%] sm:w-full max-w-sm bg-white/20 backdrop-transparent-lg rounded-2xl shadow-2xl border border-white/20 overflow-y-auto max-h-[95vh] animate-fadeInZoom">
          {/* Logo Area */}
              ...
              <input
                ...
                placeholder={
                  role === "student"
                    ...
                    : role === "staff"
                    ? t("staffEmailPlaceholder")
                    : role === "supervisor"
                    ? t("supervisorEmailPlaceholder")
                    : t("maintenanceEmailPlaceholder")
                }
                value={email}
